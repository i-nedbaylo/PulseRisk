...
- Testcontainers-сценарии `client -> account -> trade -> position`, batch insert котировок и `exposure breach -> risk alert` на настоящей PostgreSQL, которые автоматически пропускаются, если Docker Engine или локальный PostgreSQL image недоступны;
