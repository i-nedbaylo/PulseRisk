...
- `DropNewest` - новый элемент отбрасывается, если очередь переполнена и нужно сохранить уже накопленную очередь;
- `DropWrite` - новые котировки пропускаются, если важна стабильность потребителей.

Для демо рекомендуется режим `DropOldest`: в риск-мониторинге чаще важна
актуальная цена, а не обработка каждой исторической котировки. Режим
`SampleLatest`, где Risk Engine берет последнюю котировку с ограничением частоты
пересчета, остается будущей оптимизацией.
...
| --- | --- | --- | --- |
| **Strategy** | Risk rules: `MaxExposure`, `MaxLoss`, `MarginLevel`; планируются: `PriceSpike`, `HighFrequencyActivity` | Разные алгоритмы проверки лимитов должны расширяться и тестироваться независимо | Это основной GoF-паттерн проекта; он используется только для реальной алгоритмической вариативности |
